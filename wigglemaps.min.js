...
j?j["fill-opacity"]:f.style("fill-opacity");g.repeat("alpha",[a],b,c);a="stroke-opacity"in j?j["stroke-opacity"]:f.style("stroke-opacity");m.repeat("alpha",[a],d,e)},l=[];c=0;$.each(this.geom,function(a,b){for(var d,e=0;100>e;)try{for(var h=b,f=[],g=0;g<h.length;g++){for(var m=[],p=1;p<h[g].length;p++)m.push(Ea(h[g][p][0],h[g][p][1]));m.push(f[0]);f.push(m)}d=Sa(f);break}catch(fa){e++}if(100==e)throw"Rendering Polygon Failed";c+=d.length/2;l.push(d)});var k=new vect(Infinity,Infinity),x=new vect(-Infinity,
...
c&&console.log("rendering polygon failed")}),b=w):b=void 0;a.success(b)}})}})},GeoJSON:function(a,b){for(var c=0,d=0,e=0,h=[],g=[],j=[],f=0;f<a.features.length;f++){var k=a.features[f];if("Feature"==k.type){"Point"==k.geometry.type&&(c++,h.push({type:"Point",geom:[k.geometry.coordinates],attr:k.properties}));"MultiPoint"==k.geometry.type&&(c+=k.geometry.cooordinates.length,h.push({type:"Point",geom:k.geometry.coordinates,attr:k.properties}));if("Polygon"==k.geometry.type){for(var l=k.geometry.coordinates,
n=[],q=0;q<=l.length-1;q++){for(var t=[],s=l[q].length-1;0<=s;s--)t.push(l[q][s]);n.push(t)}d++;g.push({type:"Polygon",geom:[n],attr:k.properties})}if("MultiPolygon"==k.geometry.type){var u=[];$.each(k.geometry.coordinates,function(a,b){for(var c=[],d=0;d<=b.length-1;d++){for(var e=[],f=b[d].length-1;0<=f;f--)e.push(b[d][f]);c.push(e)}u.push(c)});d++;g.push({type:"Polygon",geom:u,attr:k.properties})}"MultiLineString"==k.geometry.type&&$.each(k.geometry.coordinates,function(a,b){e++;j.push({type:"Line",
geom:b,attr:k.properties})})}}if(0<c){var v=new na(c);$.each(h,function(a,b){v.append(b)});return v}if(0<d)return v=new oa(b),$.each(g,function(a,b){for(var c=0;100>c;)try{v.append(b),c=101}catch(d){c++}100==c&&console.log("rendering polygon failed")}),v;if(0<e){var w=new Ga;$.each(j,function(a,b){w.append(b)});return w}},KML:function(a){var b=$(a).find("LatLonBox"),c=new vect(parseFloat(b.find("west").text()),parseFloat(b.find("south").text())),b=new vect(parseFloat(b.find("east").text()),parseFloat(b.find("north").text()));
a=$(a).find("GroundOverlay Icon href").text();return new Wa(y+a,c,b)},SparseGrid:function(a,b){var c=ea(a,0),d=ea(a,4),e=H(a,8),h=H(a,12),g=ea(a,16),j={};for(key in b)j[key]=b[key];j.lower=new vect(c,d);j.upper=new vect(c+g*e,d+g*h);j.rows=h;j.cols=e;c=new qa(j);for(d=20;d<a.length;){for(var g=H(a,d),e=H(a,d+4),h=d+4,j=e,f=0;f<j;f++){var k=ea(a,h+4*f);c.raw_set(g+f,k)}d+=8+4*e}return c},AsciiGrid:function(a,b){var c=a.split("\n"),d=c.splice(0,6),e=parseInt(d[0].slice(14)),h=parseInt(d[1].slice(14)),
g=parseFloat(d[2].slice(14)),j=parseFloat(d[3].slice(14)),f=parseFloat(d[4].slice(14)),d=d[5].slice(14),k={};for(key in b)k[key]=b[key];k.lower=new vect(g,j);k.upper=new vect(g+f*e,j+f*h);k.rows=h;k.cols=e;g=new qa(k);for(j=0;j<h;j++){f=c[j].split(" ");for(k=0;k<e;k++)f[k]==d?g.set(h-1-j,k,NaN):g.set(h-1-j,k,parseFloat(f[k]))}return g},WMS:function(a){a=n(a);for(var b=["url","layer"],c=0;c<b.length;c++){var d=b[c];if(!(d in a))throw"Key "+d+" not found";}k(a,{levels:16,size:256});var b={source:"wms"},
e;for(e in b)a[e]=b[e];return new ga(a)}},util:{fcolor:ua,icolor:function(a,b,c,d){return new t(j(a/255,0,1),j(b/255,0,1),j(c/255,0,1),j(d/255,0,1))}},widget:{Slider:function(a,b,c){var d=function(a){if(a>=c)throw"Slider index out of bounds";return b.x/(c-1)*a},e=function(d){return d<=a.x?0:d>=a.x+b.x?c-1:Math.round((d-a.x)/b.x*(c-1))};this.dom=$("<div></div>").addClass("slider-container").css("position","relative").css("left",a.x).css("top",a.y).css("width",b.x).css("height",b.y);var h=!1,g=0,j=
$("<div></div>").addClass("slider-box").css("position","relative").css("left",-10).css("height",b.y).css("width",20).mousedown(function(){h=!0});this.tick=function(){return g};this.count=function(){return c};var f=function(){};this.change=function(a){f=a};var k=function(){};this.release=function(a){k=a};this.dragging=function(){return h};this.dom.append(j);this.set=function(a){a!=g&&f(a);g=a;var b=d(a);j.css("left",b-10);k(a)};$(document).bind("mouseup",function(){if(h){h=!1;var a=e(event.clientX);
k(a)}});$(document).bind("mousemove",function(a){h&&(a=e(a.clientX),a!=g&&f(a),g=a,a=d(a),j.css("left",a-10))})}},ready:function(a){La.push(a)}};$(document).ready(function(){$('script[src*="wigglemaps"]').each(function(a,b){var c=/wigglemaps(\.min)?\.js/;$(b).attr("src").match(c)&&(y=$(b).attr("src").replace(c,""))});$.each(La,function(a,b){b()})})})();
