...
Math.PI/4/(2*Math.PI));altitude_shader.data("screen",a.camera.mat3);altitude_shader.data("pos",j);altitude_shader.data("tex_in",k);altitude_shader.data("elevation",f);a=vect.sub(a.camera.screen(b),a.camera.screen(d));altitude_shader.data("pix_w",1/a.x);altitude_shader.data("pix_h",1/a.y);gl.drawArrays(gl.TRIANGLES,0,j.numItems);return g}}}function ra(a){v||(v=B(y+"shaders/hillshade"));var e=$(a).find("LatLonBox"),d=new vect(parseFloat(e.find("west").text()),parseFloat(e.find("south").text())),b=new vect(parseFloat(e.find("east").text()),
parseFloat(e.find("north").text()));a=$(a).find("GroundOverlay Icon href").text();var c=!1,f=ba(a,function(){c=!0});this.ready=function(){return c};var k=G(u(new vect(0,1),new vect(1,0)),2),j=G(u(d,b),2),g=315;this.draw=function(a){if(c){for(gl.useProgram(v);1<=g;)g-=1;v.data("azimuth",g);v.data("altitude",Math.PI/4/(2*Math.PI));v.data("screen",a.camera.mat3);v.data("pos",j);v.data("tex_in",k);v.data("elevation",f);a=vect.sub(a.camera.screen(b),a.camera.screen(d));v.data("pix_w",1/a.x);v.data("pix_h",
1/a.y);gl.drawArrays(gl.TRIANGLES,0,j.numItems);return g}}}function da(a){for(var e=[],d=[],b=0;b<a.levels;b++){var c=p(a);"file"==c.source&&(c.url+="/"+a.size*Math.pow(2,b+1));c.min=new vect(-180,-90);c.rows=Math.pow(2,b);c.cols=2*c.rows;c.cellsize=180/c.rows;c.z_index=1-sa-b/1E3;c.available=d;c=new Za(c);e.push(c)}sa+=(a.levels+2)/1E3;var f=null;this.initialize=function(a){A||(A=B(a.gl,y+"shaders/tile"));f=new O(a.gl,6*X);f.create("vert",3);f.create("tex",2);f.create("lookup",1);f.alloc(6*X);for(var b=
0;25>b;b++)d.push(new ia(a.gl));for(b=0;b<e.length;b++)e[b].initialize(a);e[0].fetch_all();e[0].noexpire(!0)};this.draw=function(b,c){gl.useProgram(A);A.data("screen",b.camera.mat3);Ia=Ja=0;for(var d=Infinity,l=e[0],n,h=0;h<e.length;h++){var m=a.size*e[h].cols/e[h].size(b).x;1>m&&(m=1/m);m<d&&(d=m,l=e[h],n=h)}for(h=n;0<=h;h--)e[h].fetch();if(l.ready())l.draw(b,c,f,0,!0);else{b.enable_z();d=0;for(h=n;0<=h;h--)d=e[h].draw(b,c,f,d,0==h);b.disable_z()}$.each(e,function(a,b){b.cull()})}}function Za(a){a||
(a={});a.desaturate||(a.desaturate=0);a.darken||(a.darken=0);a.hue||(a.hue=0);a.hue_color||(a.hue_color=va(0,0,0,1));if(!a.available){a.available=[];for(var e=0;8>e;e++)a.available.push(new ia(d))}var d=null,b=a.url,c=a.min,f=a.rows,k=a.cols,j=a.cellsize;this.rows=f;this.cols=k;for(var g={},l={},n=function(b,d){var e=new vect(c.x+b*j,c.y+d*j),f=new vect(c.x+(b+1)*j,c.y+(d+1)*j),e={vert:u(e,f,a.z_index),tex:null,i:b,j:d,id:b+k*d,ready:!1,min:e,max:f};l[b][d]=e;g[e.id]=e},e=0;e<k;e++)l[e]={};this.size=
function(a){var b=a.camera.screen(c);a=a.camera.screen(vect.add(c,new vect(j*k,j*f)));b=vect.sub(a,b);b.y=Math.abs(b.y);return b};var h=!1;this.noexpire=function(a){h=a};this.cull=function(){if(!h){var b=(new Date).getTime(),c;for(c in p)if(b-p[c]>$a){var d=g[c];delete g[c];delete l[d.i][d.j];a.available.push(d.tex);d.tex=null;d.ready=!1;delete p[c]}}};var m=function(){var a=engine.camera.project(new vect(engine.canvas.offset().left,engine.canvas.offset().top+engine.canvas.height())),b=engine.camera.project(new vect(engine.canvas.offset().left+
...
a.source){f={service:"wms",version:"1.1.0",request:"GetMap",layers:a.layer,bbox:[l[c][e].min.x,l[c][e].min.y,l[c][e].max.x,l[c][e].max.y].join(),width:a.size,height:a.size,srs:"EPSG:4326",format:"image/png",transparent:"true"};var k=[],g;for(g in f)k.push(g+"="+f[g]);f=b+"?"+k.join("&")}l[c][e].tex=a.available.pop();l[c][e].tex||(l[c][e].tex=new ia(d));g=f;var j=l[c][e],n=new Image;n.crossOrigin="";n.onload=function(){j.tex&&(j.tex.image(n),j.ready=!0)};n.src=g}};this.fetch_all=function(){for(var a=
(new Date).getTime(),b=0;b<k;b++)for(var c=0;c<f;c++)l[b][c]||n(b,c),l[b][c].tex||q(b,c),p[l[b][c].id]=a};this.fetch=function(){for(var a=m(),b=(new Date).getTime(),c=Math.max(0,a.min_col-1);c<Math.min(k,a.max_col+1);c++)for(var d=Math.max(0,a.min_row-1);d<Math.min(f,a.max_row+1);d++)l[c][d]||n(c,d),l[c][d].tex||q(c,d),p[l[c][d].id]=b};var z=!1;this.initialize=function(a){if(z)throw"Not Implemented: Migrating Tile Layers to New Map";A||(A=B(a.gl,y+"shaders/tile"));d=a.gl;z=!0};this.draw=function(b,
c,e,g,j){z||this.initialize(b);b=function(){Ia++;e.update();A.data("pos",e.get("vert"));A.data("tex_in",e.get("tex"));A.data("lookup_in",e.get("lookup"));A.data("desaturate",a.desaturate);A.data("darken",a.darken);A.data("hue",a.hue);A.data("hue_color",a.hue_color.array);d.drawArrays(d.TRIANGLES,0,6*g)};c=m();for(var n=(new Date).getTime(),h=Math.max(0,c.min_col);h<Math.min(k,c.max_col);h++)for(var r=Math.max(0,c.min_row);r<Math.min(f,c.max_row);r++)if(l[h][r]&&l[h][r].ready&&l[h][r].tex){e.write("vert",
l[h][r].vert,6*g,6);e.write("tex",u(new vect(0,0),new vect(1,1)),6*g,6);e.repeat("lookup",[g/X+1/(2*X)],6*g,6);p[l[h][r].id]=n;A.data("sampler"+g,l[h][r].tex);if(null==l[h][r].tex)throw"badness";g++;Ja++;g>=X&&(b(),g=0)}j&&b();return g}}var y="",R=jQuery,ab=function(a){if("string"===typeof a.data){var e=a.handler,d=a.data.toLowerCase().split(" ");a.handler=function(a){if(!(this!==a.target&&(/textarea|select/i.test(a.target.nodeName)||"text"===a.target.type))){var c="keypress"!==a.type&&R.hotkeys.specialKeys[a.which],
f=String.fromCharCode(a.which).toLowerCase(),k="",j={};a.altKey&&"alt"!==c&&(k+="alt+");a.ctrlKey&&"ctrl"!==c&&(k+="ctrl+");a.metaKey&&(!a.ctrlKey&&"meta"!==c)&&(k+="meta+");a.shiftKey&&"shift"!==c&&(k+="shift+");c?j[k+c]=!0:(j[k+f]=!0,j[k+R.hotkeys.shiftNums[f]]=!0,"shift+"===k&&(j[R.hotkeys.shiftNums[f]]=!0));c=0;for(f=d.length;c<f;c++)if(j[d[c]])return e.apply(this,arguments)}}}};R.hotkeys={version:"0.8",specialKeys:{8:"backspace",9:"tab",13:"return",16:"shift",17:"ctrl",18:"alt",19:"pause",20:"capslock",
...
2:"@",3:"#",4:"$",5:"%",6:"^",7:"&",8:"*",9:"(","0":")","-":"_","=":"+",";":":","'":'"',",":"<",".":">","/":"?","\\":"|","[":"{","]":"}"}};R.each(["keydown","keyup","keypress"],function(){R.event.special[this]={add:ab}});var I=jQuery,ta=function(a){var e=a||window.event,d=[].slice.call(arguments,1),b=0,c=0,f=0;a=I.event.fix(e);a.type="mousewheel";e.wheelDelta&&(b=e.wheelDelta/120);e.detail&&(b=-e.detail/3);f=b;void 0!==e.axis&&e.axis===e.HORIZONTAL_AXIS&&(f=0,c=-1*b);void 0!==e.wheelDeltaY&&(f=e.wheelDeltaY/
120);void 0!==e.wheelDeltaX&&(c=-1*e.wheelDeltaX/120);d.unshift(a,b,c,f);return(I.event.dispatch||I.event.handle).apply(this,d)},S=["DOMMouseScroll","mousewheel"];if(I.event.fixHooks)for(var K=S.length;K;)I.event.fixHooks[S[--K]]=I.event.mouseHooks;I.event.special.mousewheel={setup:function(){if(this.addEventListener)for(var a=S.length;a;)this.addEventListener(S[--a],ta,!1);else this.onmousewheel=ta},teardown:function(){if(this.removeEventListener)for(var a=S.length;a;)this.removeEventListener(S[--a],
ta,!1);else this.onmousewheel=null}};I.fn.extend({mousewheel:function(a){return a?this.bind("mousewheel",a):this.trigger("mousewheel")},unmousewheel:function(a){return this.unbind("mousewheel",a)}});for(var Ka=0,K=["ms","moz","webkit","o"],Y=0;Y<K.length&&!window.requestAnimationFrame;++Y)window.requestAnimationFrame=window[K[Y]+"RequestAnimationFrame"],window.cancelAnimationFrame=window[K[Y]+"CancelAnimationFrame"]||window[K[Y]+"CancelRequestAnimationFrame"];window.requestAnimationFrame||(window.requestAnimationFrame=
...
N={x:0,y:0},x=null,Aa=null,U=null,w=null,bb=wa(0,0,1,1),E=null,L=null,W={Point:{geometry:function(a,e){ua.call(this,a,e);this.bounds=null;for(var d=0;d<this.geom.length;d++){var b=new vect(this.geom[d][0],this.geom[d][1]),b=new V(b.clone(),b.clone());this.bounds?this.bounds.union(b):this.bounds=b}this.map_contains=function(a,b){for(var d=this.compute("radius"),e=0;e<this.geom.length;e++){var g=a.camera.screen(new vect(this.geom[e][0],this.geom[e][1]));if(vect.dist(g,b)<d)return!0}return!1}},renderer:function(a,
e){na.call(this,a,e);w||(w=B(a.gl,y+"shaders/point"));var d=10,b=new O(a.gl,1024);b.create("vert",2);b.create("unit",2);b.create("stroke_width",1);b.create("rad",1);b.create("fill_color",3);b.create("fill",1);b.create("stroke_color",3);b.create("stroke",1);b.create("alpha",1);var c=function(a){oa.call(this,a,e);var c,j;this.style_map={fill:function(a){"none"==a?b.repeat("fill",[-0.75],c,j):(b.repeat("fill",[0.75],c,j),b.repeat("fill_color",a.array,c,j))},opacity:function(a){b.repeat("alpha",[a],c,
j)},radius:function(a){a>d&&(d=a);b.repeat("rad",[a],c,j)},stroke:function(a){"none"==a?b.repeat("stroke",[-0.75],c,j):(b.repeat("stroke",[0.75],c,j),b.repeat("stroke_color",a.array,c,j))},"stroke-width":function(a){b.repeat("stroke_width",[a],c,j)}};a=a.geom;j=6*a.length;c=b.alloc(j);$.each(a,function(a,d){b.repeat("vert",d,c+6*a,6);b.write("unit",bb,c+6*a,6)});this.update_all()};this.view_factory=function(a){return new c(a,e)};this.draw=function(){b.update();gl.useProgram(w);w.data("screen",a.camera.mat3);
w.data("pos",b.get("vert"));w.data("circle_in",b.get("unit"));w.data("color_in",b.get("fill_color"));w.data("stroke_color_in",b.get("stroke_color"));w.data("alpha_in",b.get("alpha"));w.data("fill_in",b.get("fill"));w.data("stroke_in",b.get("stroke"));w.data("aspect",a.canvas.width()/a.canvas.height());w.data("pix_w",2/a.canvas.width());w.data("rad",b.get("rad"));w.data("stroke_width_in",b.get("stroke_width"));w.data("max_rad",d);gl.drawArrays(gl.TRIANGLES,0,b.count())}},collection:function(a){var e=
[],d=0;$.each(a,function(a,b){var k=b.compute("radius");k>d&&(d=k);$.each(b.geom,function(a,c){e.push({x:c[0],y:c[1],ref:b})})});var b=new Ea(e);this.search=function(a){a=b.search(a);var d=[];$.each(a,function(a,b){d.push(b.ref)});return new t(d)};this.map_contains=function(a,e){for(var k=vect.add(e,new vect(-d,d)),j=vect.add(e,new vect(d,-d)),k=new V(a.camera.project(k),a.camera.project(j)),k=b.search(k),j=0;j<k.length;j++){var g=k[j];if(g.ref.map_contains(a,e))return new t([g.ref])}return new t([])}}},
Polygon:{geometry:function(a,e){ua.call(this,a,e);this.bounds=Ha(this.geom);this.map_contains=function(a,b){return this.contains(a.camera.project(b))};this.contains=function(a){var b=0;if(this.bounds.contains(a)){b++;for(b=0;b<this.geom.length;b++){var c=0;$.each(this.geom[b],function(b,e){for(var j=0;j<e.length;j++){var g=(j+1)%e.length;if(0>(a.y-e[j][1])/(a.y-e[g][1])){var l=new vect(720,a.y),n=new vect(e[j][0],e[j][1]),g=new vect(e[g][0],e[g][1]);vect.intersects(a,l,n,g)&&c++}}});if(1==c%2)return!0}}return!1}},
renderer:function(a,e){na.call(this,a,e);L||(L=B(a.gl,y+"shaders/poly"));var d=new Fa(a,e),b=new O(a.gl,1024);b.create("vert",2);b.create("color",3);b.create("alpha",1);var c=function(a){oa.call(this,a,e);d.create(a);var c,j;this.style_map={fill:function(a){b.repeat("color",a.array,c,j)},"fill-opacity":function(a){b.repeat("alpha",[a],c,j)}};a=a.geom;var g=[];j=0;$.each(a,function(a,b){for(var c,d=0;100>d;)try{for(var e=b,f=[],k=0;k<e.length;k++){for(var l=[],h=1;h<e[k].length;h++)l.push(La(e[k][h][0],
e[k][h][1]));l.push(f[0]);f.push(l)}c=Wa(f);break}catch(m){d++}if(100==d)throw"Rendering Polygon Failed";j+=c.length/2;g.push(c)});var l=c=b.alloc(j);$.each(g,function(a,c){var d=c.length/2;b.write("vert",c,l,d);l+=d});this.update_all()};this.view_factory=function(a){return new c(a,e)};this.draw=function(){b.update();gl.useProgram(L);L.data("screen",a.camera.mat3);L.data("pos",b.get("vert"));L.data("color_in",b.get("color"));L.data("alpha_in",b.get("alpha"));gl.drawArrays(gl.TRIANGLES,0,b.count());
d.draw()}},collection:function(a){for(var e=[],d=0;d<a.length;d++)$.each(a[d].geom,function(b,c){$.each(c,function(b,c){$.each(c,function(b,c){e.push({ref:a[d],x:c[0],y:c[1]})})})});tree=new Ea(e);this.search=function(b){var c=tree.search(b),d={};$.each(c,function(a,b){d[b.ref.id]=b.ref});for(c=0;c<a.length;c++)for(var e=0;4>e;e++)a[c].contains(b.vertex(e))&&(d[a[c].id]=a[c]);b=[];for(var j in d)b.push(d[j]);return new t(b)};this.map_contains=function(a,c){return this.contains(a.camera.project(c))};
this.contains=function(b){for(var c=[],d=0;d<a.length;d++)a[d].contains(b)&&c.push(a[d]);return new t(c)}}},Line:{geometry:function(a,e){ua.call(this,a,e);this.bounds=Ha(this.geom);this.map_contains=function(){return!1}},renderer:Fa,collection:function(){this.search=function(){return new t([])};this.map_contains=function(){return new t([])};this.contains=function(){return new t([])}}}},ua=function(a,e){var d={},b=null;this.id=Ma();this.type=a.type;var c=a.attr;this.attr=function(a,b){if(2>arguments.length)return c[a]};
this.geom=a.geom;this.geometry=function(){};this.initialize=function(a){b=a.create(this);b.update_all()};this.compute=function(a){return za(this,e,a)};this.style=function(a,c,e){if(2>arguments.length)return void 0!==d[a]?d[a]:null;d[a]=c;b&&b.update(a)}},Ma,Na=1;Ma=function(){var a=Na;Na++;return a};var La;La=function(a,e){return new vect(a+1E-6*Math.random()-5E-7,e+1E-6*Math.random()-5E-7)};var D=null,t=function(a){var e=null;this.length=a.length;this.count=function(){return a.length};this.items=
function(){return a};this.type=function(b){for(var c=[],d=0;d<a.length;d++)a[d].type==b&&c.push(a[d]);return new t(c)};this.join=function(b){for(var c=[],d=0;d<a.length;d++)c.push(a[d]);for(d=0;d<b.count();d++){var e=b.get(d);this.id(e.id)||c.push(e)}return new t(c)};this.attr=function(){throw"Not Implemented";};this.get=function(b){return a[b]};this.subset=function(b){for(var c=[],d=0;d<b.length;d++)c.push(a[b[d]]);return new t(c)};this.id=function(b){if(!e){e={};for(var c=0;c<a.length;c++)e[a[c].id]=
a[c]}return b in e?e[b]:null};this.each=function(b){for(var c=0;c<a.length;c++)b(c,a[c]);return this};var d={">":function(a,c){return a>c},"<":function(a,c){return a<c},"==":function(a,c){return a==c},">=":function(a,c){return a>=c},"<=":function(a,c){return a<=c}};this.select=function(b){if(b.match(/^\s*\*\s*$/))return new t(a);var c=b.match(/^\s*([^\s]+)\s*([=<>]+)\s*(([^\s])+)\s*$/);if(!c)throw"Bad Selector";b=c[1];var e=c[2],k=null,j=null;isNaN(c[3])?j=c[3]:k=parseFloat(c[3]);new_elem=[];if(j)for(c=
0;c<a.length;c++)d[e](a[c].attr(b),a[c].attr(j))&&new_elem.push(a[c]);else for(c=0;c<a.length;c++)d[e](a[c].attr(b),k)&&new_elem.push(a[c]);return new t(new_elem)};this.filter=function(b){for(var c=[],d=0;d<a.length;d++)b(a[d])&&c.push(a[d]);return new t(c)};this.quantile=function(b,c,d){a.sort(function(a,c){return a.attr(b)-c.attr(b)});var e=Math.round(c*this.length/d);c=Math.round((c-1)*this.length/d);return new t(a.slice(c,e))};this.range=function(b){for(var c=Infinity,d=-Infinity,e=0;e<a.length;e++)c>
a[e].attr(b)&&(c=a[e].attr(b)),d<a[e].attr(b)&&(d=a[e].attr(b));return{min:c,max:d}};this.style=function(b,c){if(1==arguments.length){var d=[];$.each(a,function(a,c){d.push(c.style(b))});return d}if("function"==typeof c)$.each(a,function(a,d){d.style(b,c(d))});else{var e;a:if(null==c||void 0==c.length)e=!1;else{for(e=0;e<c.length;e++)if(!(e in c)){e=!1;break a}e=!0}e?$.each(a,function(a,d){d.style(b,c[a])}):$.each(a,function(a,d){d.style(b,c)})}return this}},J=null,v=null,M=null,v=null,$a=1E3,sa=
0,X=8,Ja=0,Ia=0,A=null,Oa=[];window.wiggle={Map:function(a,e){this.center=function(a,c){engine.camera.position(new vect(a,c))};this.vcenter=function(a){this.center(a.x,a.y)};this.extents=function(a){engine.camera.extents(a)};this.append=function(a){a.initialize(engine);engine.scene.push(a)};this.remove=function(a){for(var c=0;c<engine.scene.length;c++)if(engine.scene[c]==a)return engine.scene.splice(c,1),!0;return!1};this.shade=function(a){a=new ra(a);engine.shade=a};this.select=function(a){a?(engine.sel.select(a),
engine.select(!0)):engine.select(!1)};this.resize=function(){engine.resize()};this.attr=function(a,c){engine.attr(a,c)};this.png=function(){var a=engine.canvas.get(0).toDataURL();$.ajax({url:"../server/export.png",type:"POST",data:a})};this.width=function(){return engine.canvas.innerWidth()};this.height=function(){return engine.canvas.innerHeight()};var d=null;this.click=function(a){d=a};engine=new Sa(a,this,e);engine.canvas.click(function(a){d&&(a=new vect(a.pageX,a.pageY),a=engine.camera.project(a),
d(a))})},layer:{Layer:Ga,Grid:qa,Hillshade:ra,Elevation:function(a){M||(M=B(y+"shaders/elevation"));var e=$(a).find("LatLonBox"),d=new vect(parseFloat(e.find("west").text()),parseFloat(e.find("south").text())),b=new vect(parseFloat(e.find("east").text()),parseFloat(e.find("north").text()));a=$(a).find("GroundOverlay Icon href").text();var c=ba(a),f=G(u(new vect(0,1),new vect(1,0)),2),k=G(u(d,b),2);this.draw=function(a,e,l){l||(gl.useProgram(M),M.data("screen",a.camera.mat3),M.data("pos",k),M.data("tex_in",
f),M.data("elevation",c),vect.sub(a.camera.screen(b),a.camera.screen(d)),gl.drawArrays(gl.TRIANGLES,0,k.numItems))}}},io:{Shapefile:function(a){var e=a.path;$.ajax({url:e+".shx",mimeType:"text/plain; charset=x-user-defined",success:function(d){for(var b=[],c=100;c<d.length;)b.push(2*ga(d,c)),c+=8;$.ajax({url:e+".shp",mimeType:"text/plain; charset=x-user-defined",success:function(c){for(var d=[],e=[],g=0;g<b.length;g++){var l=b[g];ga(c,l);ga(c,l+4);var h=l+8,m=F(c,h);if(0==m)console.log("NULL Shape");
else if(1==m)m=Z(c,h+4),h=Z(c,h+12),d.push({attr:{},geom:[[m,h]]});else if(5==m){for(var m=F(c,h+36),h=F(c,h+40),p=l+52,l=l+52+4*m,q=[],s=0;s<m;s++){var z=F(c,p+4*s),t;t=s+1<m?F(c,p+4*(s+1)):h;var u=l,w=[];for(t-=1;t>=z;t--){var x=Z(c,u+16*t),y=Z(c,u+16*t+8);w.push([x,y])}q.push(w)}e.push({attr:{},geom:[q]})}else throw"Not Implemented: "+m;}if(0<d.length){var v=new PointLayer(d.length);$.each(d,function(a,b){v.append(b)});c=v}else 0<e.length?(v=new PolygonLayer(a),$.each(e,function(a,b){for(var c=
0;100>c;)try{v.append(b),c=101}catch(d){c++}100==c&&console.log("rendering polygon failed")}),c=v):c=void 0;a.success(c)}})}})},GeoJSON:function(a,e){void 0===e&&(e={});for(var d=new Ga(e),b=0;b<a.features.length;b++){var c=a.features[b];if("Feature"==c.type){"Point"==c.geometry.type&&d.append({type:"Point",geom:[c.geometry.coordinates],attr:c.properties});"MultiPoint"==c.geometry.type&&d.append({type:"Point",geom:c.geometry.coordinates,attr:c.properties});if("Polygon"==c.geometry.type){for(var f=
c.geometry.coordinates,k=[],j=0;j<=f.length-1;j++){for(var g=[],h=f[j].length-1;0<=h;h--)g.push(f[j][h]);k.push(g)}d.append({type:"Polygon",geom:[k],attr:c.properties})}if("MultiPolygon"==c.geometry.type){var n=[];$.each(c.geometry.coordinates,function(a,c){for(var b=[],d=0;d<=c.length-1;d++){for(var e=[],f=c[d].length-1;0<=f;f--)e.push(c[d][f]);b.push(e)}n.push(b)});d.append({type:"Polygon",geom:n,attr:c.properties})}"MultiLineString"==c.geometry.type&&$.each(c.geometry.coordinates,function(a,b){d.append({type:"Line",
geom:[[b]],attr:c.properties})})}}return d},KML:function(a){var e=$(a).find("LatLonBox"),d=new vect(parseFloat(e.find("west").text()),parseFloat(e.find("south").text())),e=new vect(parseFloat(e.find("east").text()),parseFloat(e.find("north").text()));a=$(a).find("GroundOverlay Icon href").text();return new Ya(y+a,d,e)},SparseGrid:function(a,e){var d=aa(a,0),b=aa(a,4),c=F(a,8),f=F(a,12),k=aa(a,16),j={};for(key in e)j[key]=e[key];j.lower=new vect(d,b);j.upper=new vect(d+k*c,b+k*f);j.rows=f;j.cols=c;
d=new qa(j);for(b=20;b<a.length;){for(var k=F(a,b),c=F(a,b+4),f=b+4,j=c,g=0;g<j;g++){var h=aa(a,f+4*g);d.raw_set(k+g,h)}b+=8+4*c}return d},AsciiGrid:function(a,e){var d=a.split("\n"),b=d.splice(0,6),c=parseInt(b[0].slice(14)),f=parseInt(b[1].slice(14)),k=parseFloat(b[2].slice(14)),j=parseFloat(b[3].slice(14)),g=parseFloat(b[4].slice(14)),b=b[5].slice(14),h={};for(key in e)h[key]=e[key];h.lower=new vect(k,j);h.upper=new vect(k+g*c,j+g*f);h.rows=f;h.cols=c;k=new qa(h);for(j=0;j<f;j++){g=d[j].split(" ");
for(h=0;h<c;h++)g[h]==b?k.set(f-1-j,h,NaN):k.set(f-1-j,h,parseFloat(g[h]))}return k},WMS:function(a){a=p(a);for(var e=["url","layer"],d=0;d<e.length;d++){var b=e[d];if(!(b in a))throw"Key "+b+" not found";}h(a,{levels:16,size:256});var e={source:"wms"},c;for(c in e)a[c]=e[c];return new da(a)}},util:{fcolor:va,icolor:function(a,e,d,b){return new s(m(a/255,0,1),m(e/255,0,1),m(d/255,0,1),m(b/255,0,1))}},widget:{Slider:function(a,e,d){var b=function(a){if(a>=d)throw"Slider index out of bounds";return e.x/
(d-1)*a},c=function(b){return b<=a.x?0:b>=a.x+e.x?d-1:Math.round((b-a.x)/e.x*(d-1))};this.dom=$("<div></div>").addClass("slider-container").css("position","relative").css("left",a.x).css("top",a.y).css("width",e.x).css("height",e.y);var f=!1,h=0,j=$("<div></div>").addClass("slider-box").css("position","relative").css("left",-10).css("height",e.y).css("width",20).mousedown(function(){f=!0});this.tick=function(){return h};this.count=function(){return d};var g=function(){};this.change=function(a){g=
a};var l=function(){};this.release=function(a){l=a};this.dragging=function(){return f};this.dom.append(j);this.set=function(a){a!=h&&g(a);h=a;var c=b(a);j.css("left",c-10);l(a)};$(document).bind("mouseup",function(){if(f){f=!1;var a=c(event.clientX);l(a)}});$(document).bind("mousemove",function(a){f&&(a=c(a.clientX),a!=h&&g(a),h=a,a=b(a),j.css("left",a-10))})}},ready:function(a){Oa.push(a)}};$(document).ready(function(){$('script[src*="wigglemaps"]').each(function(a,e){var d=/wigglemaps(\.min)?\.js/;
$(e).attr("src").match(d)&&(y=$(e).attr("src").replace(d,""))});$.each(Oa,function(a,e){e()})})})();
